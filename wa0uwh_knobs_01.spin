...
PUB Demo | Sec, Minute, Hour, PrevSec, okay, Csync, Tsync, SpkrTone
      ...
      StartI2CKnobs(@E0, @B0, @E1, @B1, @B2, @B3)
      ...
      repeat 'Repeat Forever
        ...
        DSP1.lock
          ...
          DSP1.XyRjDec(20,2, GetCogStat//100_000, 6, " ")
        DSP1.UnLock
...
DAT
VAR 'START KNOBS
   Long cog, cogStack[64], CogStat 'Local Method Static Variables
PUB StartI2CKnobs(pE0, pB0, pE1, pB1, pB2, pB3) | okay
...
' See usage in the above Demo Method
    ...
    repeat while NOT CogStat  'Delay until COG is Established
...
VAR 'KNOB AND BUTTON ISR
     Long IncDir  'Local Method Static Variables
PRI KnobButISR(pE0, pB0, pE1, pB1, pB2, pB3)| {
     } pins, mask, ISRv, ThisVal, PrevKnobInt, PaceCnt
{{
    Program Flow
      1. Init
      2. StartLoop
      3. Wait for Button or Rotory Knob Event
           The I2C Expander is configured to Interrupt on Change for Encoders Pins and
           Interrupt on Level for Push Buttons, see StartEXP0 above.
      4. Decode Rotory Knobs and Buttons, update passed parameters as necessary
      5. Loop

    Notes:
      a. Locks makes this Multi COG Friendly, User must insert I2C Locks in their
         code. I2C Locks for this Method are performed by the EXP0 Object.
      b. To support both types of Knobs (Left and Right Increment), the Direction
         of Encoder Increment (IncDir) is Established on First Knob Event. By
         convention the First User Knob-Twist Should be to the Right.
    ...
    'Clear First Erroneous Interrupts
    EXP0.getbyte(EXP0#INTCAPA)

    'StartLoop, Loop Forever Loop
    repeat
        CogStat++
        EXP0.getbyte(EXP0#INTCAPA)
        waitpne(mask, pins, 0) 'waiting for interrupt signal
        ThisVal := ISRv := EXP0.getbyte(EXP0#INTCAPA)  'Get Interrupt Value
        DeBugVar := ISRv

        'The following is a work-a-round for bad Encoder Indents
        if PrevKnobInt & 1<<Encoder0PinA
           ISRv &= !(1<<Encoder0PinA | 1<<Encoder0PinB)
        if PrevKnobInt & 1<<Encoder1PinA
           ISRv &= !(1<<Encoder1PinA | 1<<Encoder1PinB)
        PrevKnobInt := ThisVal & (1<<Encoder0PinA | 1<<Encoder1PinA)

       'Decode Rotory Knobs and Buttons, update passed parameters as necessary
       'If not set, establish IncDir
        DeBugVar := ISRv

        'Check for an Encoder(s) Turned
        if (ISRv & KnobIntMask)
            'Check for Encoder0
        ...
        'Check For Button(s) Presses
        if (ISRv & ButtonIntMask)
            if PaceCnt-- < 0
           ...
           {if_end}
        {if_end}
        'DeBugVar := long[pE0] + long[pB0] + long[pE1] + long[pB1] + long[pB2] + long[pB3]
